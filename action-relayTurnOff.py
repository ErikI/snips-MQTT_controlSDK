...
client = mqtt.Client("voice_test2")
    ...
    if len(intentMessage.slots.house_room) > 0:
        ...
        if house_room == "living room":
            if device_name == "LED strip":
                client.publish("cmnd/tv_light_status/POWER","OFF")
                action_performed = True
        elif house_room == "kitchen":
            if device_name == "LED strip":
                client.publish("cmnd/k_light_status/POWER","OFF")
                action_performed = True
        if action_performed:
            result_sentence = "Turning off {} in : {}".format(str(device_name), str(house_room))
        else:
...
if __name__ == "__main__":
    ...
    with Hermes(mqtt_options=mqtt_opts) as h:
        h.subscribe_intent("ErikI:relayTurnOff", subscribe_intent_callback) \
         .start()